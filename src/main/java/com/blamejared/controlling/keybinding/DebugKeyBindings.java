...
import com.blamejared.controlling.api.KeyContextRegistry;
...
public final class DebugKeyBindings {
    ...
    public static void registerIfDevEnvironment() {
        ...
        KeyContextRegistry.register(DEBUG_CONTEXT);
        ...
        // Combos allowed, but never on Ctrl: exercises the per-key block list.
        final KeyBinding noCtrl = registerBind("Debug: No Ctrl Combo", Keyboard.KEY_B, KeyContexts.UNIVERSAL, true);
        ControllingApi.setBlockedComboKeys(noCtrl, Keyboard.KEY_LCONTROL, Keyboard.KEY_RCONTROL);
    ...
    private static KeyBinding registerBind(String description, int keyCode, KeyContext context, boolean allowsCombos) {
        ...
        ControllingApi.setAllowsCombos(keyBinding, allowsCombos);
        return keyBinding;
    }
}
